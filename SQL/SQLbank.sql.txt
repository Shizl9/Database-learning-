...
-- 2. Display customer full name, phone, and membership start date (account creation)
SELECT c.name, c.phone, MIN(a.creation_date) AS MembershipStartDate
FROM Customer c
JOIN Account a ON c.customer_id = a.customer_id
GROUP BY c.name, c.phone;

-- 3. Display each loan ID, amount, and type
SELECT loan_id, amount, loan_type FROM Loan;
...
-- 7. Display accounts opened in 2024
SELECT * FROM Account WHERE YEAR(creation_date) = 2024;
...
SELECT MAX(balance) AS MaxBalance, MIN(balance) AS MinBalance, AVG(balance) AS AvgBalance FROM Account;
...
-- 12. Display loans issued before 2014 (or with duration > 10 years)
SELECT * FROM Loan WHERE DATEDIFF(year, issue_date, GETDATE()) > 10;
---DML
-- 13. Insert yourself as a new customer
INSERT INTO Customer (customer_id, name, address, phone, date_of_birth)
VALUES (9003, 'Sheika', 'Muscat', '95125165', '2001-04-10');

-- 14. Insert another customer with NULL phone and address
INSERT INTO Customer (customer_id, name, address, phone, date_of_birth)
VALUES (9002, 'Test Customer', NULL, NULL, '1998-05-05');

-- 15. Increase your account balance by 20% (assume account exists)
UPDATE Account SET balance = balance * 1.20 WHERE customer_id = 9001;

-- 16. Increase balance by 5% for accounts with balance less than 5000
UPDATE Account SET balance = balance * 1.05 WHERE balance < 5000;

-- 17. Update phone number to 'Not Provided' where phone is NULL
UPDATE Customer SET phone = 'Not Provided' WHERE phone IS NULL;

-- 18. Delete closed accounts (balance = 0)
